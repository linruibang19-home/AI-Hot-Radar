...
<details>
<summary>当前生产与质量快照（2026-08-15，历史快照，非实时承诺）</summary>

- 生产版本：`v0.1.15@709378c`，香港 2C4G 单机，Docker Compose 编排，Caddy 提供 HTTPS。
- 分层回归：Python **916 passed / 2 skipped**，mypy 检查 87 个源文件无错误；Java `85 / 85` 通过；Web `81 / 81` 通过。契约、迁移、生产构建、Compose smoke 与受影响 RAG 门禁由 CI 分层执行。
