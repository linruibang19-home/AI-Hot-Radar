...
| 层 | 选型 |
|---|---|
| 前端 | Next.js 15.5.23（App Router、SSR、同源代理） |
| 业务 API | Java 21 + Spring Boot 3.4.1（读接口、报告、订阅、RBAC 审计） |
| AI 服务 | Python 3.12 + FastAPI（采集、加工、检索、问答） |
| 数据 | PostgreSQL 16 + pgvector（唯一事实源）· Redis（缓存 / 限流 / 短锁） |
| 模型 | bge-m3 嵌入 · bge-reranker-v2-m3 重排 · DeepSeek 生成（全部外置 API） |
| 交付 | GitHub Actions 五路门禁 → GHCR 不可变 `sha-<commit>` 镜像 → Docker Compose + Caddy |
