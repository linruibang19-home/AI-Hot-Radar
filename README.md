...
当前状态：**M0–M4 完成并持续自动运行**，M5（上线）待执行。
语料每 120 秒轮询一次，最近一次快照 **1538 条内容 / 5841 个分块（全部已向量化）/ 1176 个事件**。
...
站内 `/eval` 页公开了全部 **10 轮检索评测 + 2 轮生成侧评测 + 1 轮延迟实测**的逐题数据与判据，
...
**检索**（B1 纯稠密 → 当前）：MRR **0.7630 → 0.8731**，Recall@10 **0.8045 → 0.8626**
...
问题
 ...
 └─ 双通道召回   稠密 pgvector/HNSW (bge-m3, 1024 维)   稀疏 tsvector/GIN + CJK bigram
 └─ 加权 RRF 融合（dense 1.0 / sparse 0.6 / temporal 0.15）
...
- **M5 上线**：部署产物（prod compose / Caddy / GHCR workflow / 备份脚本）已就绪，
  剩下的是密钥轮换、买服务器、DNS 与恢复演练——见 `docs/status/project-status.md` §7。
...
- **查询改写没做**：廉价版本要用 `entity.aliases`，而那 4168 行全是空数组；
  服务期 LLM 改写要多一次往返，而实测 99% 的延迟就在往返上。
- **Noise Sensitivity 未测**：它需要构造带噪上下文的对照集，当前黄金集按真实语料标注。
