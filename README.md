...
当前状态：**M0–M4 功能完成并持续自动运行，发布门禁仍有未闭环项**；M5 上线待执行。
语料每 120 秒轮询一次，2026-08-10 本次实测快照为
**1750 条内容 / 6780 个分块（全部已向量化）/ 1359 个事件**。
...
**检索**（B1 纯稠密 → B15 稳定基线）：MRR **0.7630 → 0.8720**，
Recall@10 **0.8045 → 0.8509**，Recall@20 **0.8876 → 0.8908**。
...
Flyway 在 core-api 启动时自动迁移（V001–V021）。
...
本次实跑的三套单元/集成测试共 **936 个**：Python 827 · Java 55 · Web 54；
...
- **通用 LLM 查询改写默认关闭**：厂商别名、实体与多轮指代改写已经上线；
  通用改写会多一次模型往返，必须先用专项集证明端到端收益。
