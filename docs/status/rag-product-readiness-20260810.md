...
2. 发布工作流未闭环（14 份报告均为 DRAFT；读者页继续展示已生成报告，但缺人工发布端点/UI）；
...
数据库当前有 14 份报告，全部为 DRAFT。报告生成器、读者页和邮件原本以这些已保存记录
构成同一条功能链；在发布后台尚未实现时，读者接口继续返回已生成报告，并由页面明确标注
AI 自动生成。`DRAFT` 仍保留为编辑状态，不能为了显示页面把记录伪改成 `PUBLISHED`。
