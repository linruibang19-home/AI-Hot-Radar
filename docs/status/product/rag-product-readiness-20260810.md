...
> **08-10 建立、08-11 补充闭环结果的对标文档。** 其中 §2.1、§3.1 保留 08-10
> 语料/检索基线用于比较，当前运行快照请读 [08-11 交接](../history/handoff-20260811.md)、
> [专项发布审计](rag-specialist-audit-20260811.md) 与
> [安全性能验收](rag-security-performance-20260811.md)。本文的成熟产品对标框架与已勾选
> 闭环项仍有效，引用数字时必须注明是 08-10 基线还是 08-11 发布轮。
...
1. 权限与治理尚不完整（匿名公共问答、无租户/文档 ACL；08-11 已补提示注入与凭据输出基线）；
2. 报告发布与邮箱订阅已于 08-11 闭环（自动质量门、PUBLISHED-only、人工发布/撤回、
   双重确认、定时投递、退订、幂等与审计）；生产 SMTP、退信/投诉处理和浏览器管理写 UI
   仍未闭环；
3. 中文厂商名专项集、噪声 A/B 与人工 P0 核验已于 08-11 闭环；仍缺持续回归和线上反馈闭环；
...
综合判断：**检索与证据链约 4.3/5，产品化约 3/5，企业治理约 2/5；整体 3.4/5。**
...
- UI 保持原布局，新补「检索截至」与证据概览（引用数、发布方、支持度、一手来源）；
  08-10 Chrome 桌面/窄屏实测无应用控制台错误、无横向溢出，引用定位可交互；08-11
  空状态与输入区精修已通过构建/运行态，改后 Chrome 截图仍待下一次浏览器验收。
...
08-11 完整 90 题生成评测：citation coverage **0.9881**；段落级支持度均值
0.8907、达标率 **0.9344**；模型实际读取父块口径均值 0.9505、达标率
0.9872；可答题误拒率 0，12 道不可答题的断言假前提率 0。

这里最重要的事实不是「1.0000」，而是父块支持度已参与门控，因此它是门控结果、
不是独立测量。产品判据使用仍能失败的段落级 0.9344（ADR-0021）。

### 3.3 工程门禁

- Python：863 passed；Ruff check 全绿；mypy 86 个源码文件全绿。
- Web：55 passed；ESLint、TypeScript、Next.js production build 全绿。
- Java：Java 21 容器内 55 passed。
- Ruff 全库 format 已在发布基线复核中通过。
...
| 生成 | 结构化输出、引用、拒答、grounding/安全校验 | 已有引用/支持度/拒答、不可信证据边界与凭据 fail-closed；无企业 DLP/ACL |
| 评测 | 黄金集、检索/生成/延迟/成本、回归门禁、线上反馈闭环 | 离线较强；已有小规模噪声集和人工 P0，持续评测与用户反馈闭环不足 |
| 治理 | RBAC/ACL、租户隔离、审计、PII、保留/删除、发布审批 | 管理 token 与审计已有；公共问答无 ACL/租户，发布未闭环 |
| 运行 | 超时、重试、限流、缓存、熔断、SLO、告警、灾备 | 已有分阶段 p95/p99 SLO 与快速失败；真实告警、备用供应商和恢复演练未闭环 |
...
1. 实现报告人工发布端点/UI 与审计；当前读者页展示自动生成的 DRAFT，不能把“可预览”误当成“已审核发布”。
2. 轮换全部密钥、设置供应商消费上限、完成一次真实备份恢复演练。
3. ✅ 公开问答已补不可信证据边界与凭据输出 fail-closed；若引入账户，再做行级 ACL。
4. ✅ 关键问答已完成人工引用 P0 核验，见 08-11 专项审计。

### P1：直接提高 RAG 质量

1. ✅ 已新增 15 道「中文厂商名→英文产品/模型名」题和真实近邻噪声对照集。
2. ✅ 已在同一进程、同一候选快照里完成带重排 A/B，隔离 HNSW 漂移。
3. 给 `rag_query_id` 保存候选 ID/顺序/模型版本摘要，使历史回答可严格重放。
4. ✅ 已对完整性与弱支持案例逐题分析；完整性 0.9881，段落支持达标率 0.9344，
   支持阈值未下调，详见 `rag-specialist-audit-20260811.md`。
5. ✅ 专项 Recall@20 0.9333，未触发查询改写；第 27 名个案不足以证明额外在线 LLM 往返划算。

### P2：规模与运行成熟度

1. ✅ 已为 embedding/rerank/generate/support 建立 p95/p99 SLO 并收紧重排失败边界；
   备用模型仍需第二供应商真实配置与回归后才能启用。
...
08-10 审计时数据库有 14 份 DRAFT。08-11 的 TASK-M5-003 已实现确定性自动发布门、
PUBLISHED-only 公共读取、人工发布/撤回、幂等和审计；同一批 14 份报告经门禁验证后全部
转为 PUBLISHED。这里保留 08-10 的问题背景，但不再作为当前状态。
