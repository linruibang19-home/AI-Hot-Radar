...
| 指标 | 数值 |
|---|---:|
| 登记信源 / 允许调度 / 运行态 ACTIVE | 151 / 139 / 110 |
| 内容 | 2816 |
...
登记信源与允许调度两列取自仓库 `config/sources.yaml`（`registry_version 2026-08-30.2`），
**尚未部署到生产**；运行态 ACTIVE 仍是 2026-08-19 香港实查值，要等发布后重新核。
其余各行为 2026-08-19 实查。这些数字会随调度继续增长。网站、报告、邮件、RAG、订阅和审计的业务事实均来自 PostgreSQL；
