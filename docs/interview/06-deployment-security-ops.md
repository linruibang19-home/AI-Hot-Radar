...
本机主要运行 HTTP、数据库、JVM/Python、文本处理和容器；模型算力在供应商。当前低并发和约 4 万
分块下，风险先是 4GB 内存、磁盘、镜像/日志/备份增长及外部 API P95，而非 CPU 推理。

2026-09-24 实测，第一个撞上的就是**数据库内存**：`shared_buffers` 128 MB、容器上限 640 MB，而
`content_chunk` 918 MB（每行带 1024 维向量）。无时间窗的检索每次都要读盘，线上回答 11–20 s。
先做的是 SQL 执行计划修正（最坏 13.4 s → 0.8 s）；剩下的要靠调库内存与各容器限额，或把全文检索
列拆到窄表，再谈升配。需要限制
