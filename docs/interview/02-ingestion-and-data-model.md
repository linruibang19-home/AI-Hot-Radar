...
### 被问「你怎么知道采集在正常工作」时，别只说状态机

上面这套只覆盖**失败**。真正咬人的是**成功但没产出**——2026-08-30 查出 12 个 changelog
信源各自恰好 1 条内容，而它们全部 `status = SUCCESS`、`HTTP 200`、`consecutive_failures = 0`、
错误日志为空。根因是同页锚点在 URL 规范化时被丢弃导致唯一索引冲突，异常被"单条失败不拖垮
整轮"的兜底 catch 吞掉，`discovered_count` 还是真实段落数——**每一个计数都是对的，只有内容
不在**。修复后该档位 11 → 325 条。

由此得到的规则，比状态机本身更值得讲：

- **`SUCCESS + discovered_count = 0` 连续出现不该算健康**。空结果和"安静的一周"长得一模一样，
  而系统默认信后者。这个信号目前还没做，是已知缺口，答辩时如实说。
- **看分布，不要只看日志。** "每个信源都恰好 1 条""某几个恰好 0 条"这种整齐的数字是结构
  缺陷的指纹，不是内容稀少。一条 `GROUP BY source_id ORDER BY count` 就能照出来，而任何
  健康检查都照不出。
- 同批查出的其他两例：3 个 OpenAI 信源被香港 IP 403（连续失败 71–73 次，此前被"每源 1 条"
  的假象盖住），以及 `static_listing_to_article` 档位 489 条中 488 条没有发布日期——检索按
  `COALESCE(published_at, observed_at)` 裁时间窗，于是旧稿一律显示成"刚发布"。后者**故意
  没有立刻修**：最省事的回填实测会把 Anthropic 全部条目写成 2023-11-03（JSON-LD 无
  `datePublished`，回落到站点级日期），**写一个错日期比留空更糟**。
