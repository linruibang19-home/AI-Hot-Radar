...
pgvector 不是因为“永远最强”，而是当前约 4 万分块、低并发和强 SQL 过滤下最小复杂度的选择。
当单库容量、并发、构建时间或召回延迟经压测越界时，才以双写/回填/切换 ADR 迁移。

同库也有代价，而且只在生产暴露：2026-09-24 规划器把「条目⋈版本」估成 1 行（实际 7,521），关键词
检索走嵌套循环读了整张分块表，13.4 s；用 MATERIALIZED CTE 固定「先走索引再 JOIN」后 0.8 s。
稠密检索同理改为先排序再 JOIN（11.3 s → 约 2 s）。两处结果都与改前逐条一致。
