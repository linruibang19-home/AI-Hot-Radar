...
## 生产慢查询：本地 0.15 s，生产 13 s（2026-09-24）

压测测的是 HTTP 读路径；RAG 检索的慢是另一类问题，而且**只在生产出现**。完整 STAR 见
[`08-resume-and-star-stories.md`](08-resume-and-star-stories.md) 的 STAR 14。

### Q：怎么确定慢在哪一层、是不是刚上线的改动造成的？

三步排除：生产容器内只读连跑三次，稳定 13.6 s，不是冷缓存；同一容器换回上一版代码，同样 13.6 s、
结果一致，不是本次改动；再看 `EXPLAIN (ANALYZE, BUFFERS)`，确定慢在哪个执行节点。

### Q：执行计划里看到了什么？

规划器把「条目⋈版本」估成 1 行，实际 7,521 行，于是选了嵌套循环：逐个版本取出它的全部分块，
再测全文匹配——GIN 索引根本没用上，读盘 48,521 页。本地内存能装下整张 918 MB 的表，所以同样的
计划只要 0.15 s。

### Q：怎么修，怎么证明没改坏？

用 MATERIALIZED CTE 固定「先走 GIN 索引取匹配分块，再连接」，打分公式不变。本地 171 道真实问题
有/无时间窗两种情况逐行、逐分数一致；生产只读对比也逐条一致；同一查询 13.1 s → 0.8 s。补了结构
测试，防止以后被改回去。

### Q：修完就解决了吗？

没有。含高频词的问题仍要 5–10 s：命中几千个分块，每个都要从磁盘读宽行。还试过"按检索词走索引
累加 IDF"，8 题中 6 题降到 30–90 ms，但含超高频实体词的 2 题慢一倍，没有上线。剩下的瓶颈是
`shared_buffers` 128 MB 这类内存配置，或把全文检索列拆到窄表，不是再改 SQL。

## 简历数字写法

可以写（生产慢查询，推荐，已在当前投递版里）：

> 在生产用 EXPLAIN 定位规划器错估行数导致的全表扫描，改为先走索引再连接，关键词检索由 13.4 s
> 降至 0.8 s。

口径：同一查询、生产只读实测、结果逐条一致；被问到时主动补"高频词问题仍 5–10 s"。
