...
当前约 4 万分块且检索强依赖状态、时间、实体和来源 join；同库保持事务过滤一致、运维更小。容量或并发压测越界再迁。
...
父块只补语境；引用锚点在同一篇/同一章节内按「引用它的每一句」选到真正支撑的分块（ADR-0037），悬停段落支持率 0.8613 → 0.8993。支持度按每条引用的一句 claim 检查，不是逐句。
...
它是固定黄金集、语料 cutoff 和模型上的可复现发布实验，生成侧按每题提问时间冻结语料。实时请求延迟/错误在 `/ops`；混在一起无法定位变化来源。快照是否达标由 CI 校验（ADR-0038）。
...
模型外置后，优先关注内存、PostgreSQL/向量增长、镜像/日志/备份空间和外部 API P95，而非本地推理 CPU。实测第一个撞上的就是库内存：`shared_buffers` 128 MB 装不下 918 MB 的分块表，无时间窗检索每次读盘。
...
## 2026-09 新增追问（v0.1.30–v0.1.32）

这一组来自最近一轮优化，每题都有可核对的产物。回答仍按「结论 → 证据 → 边界」。

| # | 问题 | 回答主线 | 证据与边界 |
|---:|---|---|---|
| 121 | 你的评测会不会有数据泄漏？ | 有过：生成评测能看到提问日之后的内容；按 `asked_at` 冻结后引用准确率 0.5673 → 0.6888，**旧数字偏低**而非偏高 | 08 STAR 13、`GEN-FROZEN` run；检索评测一直是冻结的 |
| 122 | 谁当裁判？LLM-as-judge 可靠吗？ | 能计数的不用模型（召回、完整性）；支持度用交叉编码器；诱导题用只答是/否的 LLM 判官 | 交叉编码器测相关性不测蕴含，这是它的边界 |
| 123 | 支持度 99% 是说每句话都有依据吗？ | 不是：每条引用只打一句 claim；逐句 93%；逐句删除实测准确率 ≤26%，否决 | ADR-0037、`sentence-support-pairs` |
| 124 | 发布门禁是自动的吗？ | 是：CI 重建评测汇总并比对，7 项条件逐项校验，不过就不出发布镜像 | ADR-0038；CI 不重跑评测，忘记重跑仍靠发布纪律 |
| 125 | 怎么证明一个优化真的有效？ | 配对：同一批题、同一份草稿，只改一个变量；提升小于 ±0.02 噪声底就不算 | `audit-ab`、B14 噪声底 |
| 126 | 本地快、生产慢怎么排查？ | 生产只读 `EXPLAIN (ANALYZE, BUFFERS)`，看行数估计和读盘页数 | 08 STAR 14：13.4 s → 0.8 s |
| 127 | 线上回答为什么还要 10–20 秒？ | 生成 3–5 s；无时间窗时检索要读盘：库内存 128 MB，分块表 918 MB | 下一步调库内存或把全文列拆到窄表 |
| 128 | 模型被供应商静默升级怎么办？ | 快照记录模型名；DeepSeek 把 `v4-flash` 交给 V4.1-Flash 服务、调用方无通知；换模型必须重跑生成回归 | 名字不变时漂移检测会失效，这是边界 |
| 129 | 为什么不开 LLM 规划器？ | 复测 R@20 −0.009、0 升 2 降、每问多 0.8 s；分类更准不等于召回更好 | `PLANNER-AB` run |
| 130 | 数值审计防什么、代价多大？ | 防「64%，即 $4.65 对 $8.37」这种口径混用；改成算术不变量后不再误拒；无需修改只回 ok，3.9 s → 1.0 s | 08 STAR 16、ADR-0034/0036 |
| 131 | 引用准确率只有 66.8%？ | 标注稀疏：引用了同样相关但没被标注的文章也算"错"；只作诊断，门禁看支持率 | 需扩充标注才能当门禁 |
| 132 | 剩下约 10% 没召回的是什么？ | 分类别看，"原理解释"类最低 85.6%；准备 1–2 个具体漏召回的题当例子 | `B9-RELEASE` 逐题数据 |
...
| 60 | PostgreSQL 同时搜索会不会慢？ | 会，且本地测不出：生产关键词检索 13.4 s，规划器把条目⋈版本估成 1 行走了全表；MATERIALIZED 先走 GIN 后 0.8 s | `EXPLAIN (ANALYZE, BUFFERS)`；剩余瓶颈是 2C4G 库内存 |
| 61 | HNSW 为什么而非 IVFFlat？ | 免训练、高 recall；但无时间窗时反而不用近似：`ef_search=100` 与精确结果重合最低 0.65，改为先排序再 JOIN 的精确路径 | 生产 11.3 s → 约 2 s，结果一致 |
...
| 73 | Sparse 中文为什么难？ | simple tokenizer 缺中文词边界，用 CJK bigram：纯中文题通道 R@20 0 → 0.34；但跨词碎片在 IDF 下会压过专名 | ADR-0018；去碎片修法在 P0 专项退步，未上线 |
...
| 86 | Recall 90% 是否够？ | 过了 85% 门槛，但仍有约 10% 相关项漏召回；原理解释类最低 85.6%，需按风险看失败题 | `/eval`（0.8998）；不能称行业 SOTA |
| 87 | 引用完整性 100% 等于正确吗？ | 否，只说明有引用；段落支持 99% 按每条引用一句，逐句 93%；人工 entailment 另测 | ADR-0021、ADR-0037 |
| 88 | 自动评测何时刷新？ | 模型/Prompt/切块/检索策略变化后主动重跑并版本化；CI 校验提交的快照 | ADR-0038；CI 不重跑评测，改代码忘记重跑拦不住 |
