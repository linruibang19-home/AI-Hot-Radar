...
模型答案正确但未按 JSON/claims 契约，解析失败分支把答案清空，误拒到 7.69%。抓原始响应、做带守卫回退后 08-11 批次回到 0，并证伪了原先"语料竞争"假设。

补一句更值钱的后续：那个 0 是**一次采样**。同配置连跑四轮为 2/1/1/3，说明误拒是随机量而不是可以要求为零的属性；门禁因此从硬门禁改成阈值门禁（≤5%），只对"编造假前提"保留零容忍。**答辩时主动讲这个自我纠正，比守着 0% 有说服力。**
...
### 24H. 省略人工审核会不会导致日报和 RAG 不能发布？

不会。省略的是主题/厂商关系分类器的离线外部效度标注，不是实时内容审批。采集正文仍经过全文门、
结构化 schema、质量分与可追溯原文；日报按已入库内容生成，RAG 由 90 题自动门禁、引用绑定和拒答
保护。团队产品里会用版本化人工标签校准下一版分类器，高风险数字另进 P0 单条审核，而不是让两位
审核者卡住每篇新闻。

### 24I. 你如何证明重切块已经在真实生产完成？

V026 后只选择当前 revision 中超过 1,200 token 的 active 块：13 个 revision 的 80 个旧块退役，
生成 107 个新块；107 个全部补成 `BAAI/bge-m3` 向量。SQL 门显示超限、缺向量、active 序号重复、
chunk 等于摘要都是 0，同时 3 个 retired 块仍被历史答案引用。迁移后的 132MB 备份还在隔离库
完整恢复并核对 Flyway 026 与五张关键表计数。

### 24J. RAG 到底切的是全文还是摘要？有没有例外？

只有全文门 ACCEPTED 的 canonical revision 才进入切块；输入是 `content_revision.body_text`，摘要
存放在 `content_item.summary_zh`，两条路径分离。生产相等性检查为 0，89,338 字符长文能形成 93 个
passage。metadata-only、被拒全文、空正文和尚处 DISCOVERED 的事件不会假装成可检索全文，这些是
明确排除，不是拿摘要补位。
