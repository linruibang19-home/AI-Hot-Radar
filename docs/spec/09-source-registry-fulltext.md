...
- 118 个允许进入启动探测队列，22 个在专用适配器或授权完成前默认关闭。
...
机器之心、InfoQ 中文、36氪、雷峰网、智东西和部分厂商社区必须使用独立 fixture 与 selector。只有在普通 HTTP 无法取得正文且 robots/条款允许时，才启用浏览器渲染。禁止使用登录 Cookie 回放、验证码绕过或未授权接口。
