...
| 注册表 | 数量 | 用途 |
|---|---:|---|
| `config/sources.yaml` | 143 | 官网、更新日志、RSS、GitHub、论文、作者和媒体的可采集入口 |
...
这里的分批是部署顺序，不是继续遗漏信源。143 个可采集入口和 38 个受限监控目标已经在配置中完整登记。
...
禁止一次写 143 个硬编码爬虫类。先实现通用 profile adapter，再为 `adapter_required` 来源添加声明式 selector 与 fixture。每个站点适配器必须有：成功页、无正文页、结构变化页、429/403、发布时间缺失和 canonical 冲突测试。
