...
订阅入口由 Core API 持有，Web 只做同源代理。邮箱提交后先保存 24 小时有效的待确认请求，
确认后才建立 ACTIVE 订阅；重复申请十分钟内不重复发确认信。调度器只挑选确认时间之后的
最新 PUBLISHED 报告，并等待收件人 IANA 时区的 08:30。`(subscription_id, report_id)`
唯一键与 `FOR UPDATE SKIP LOCKED` 共同防止多实例重发；SMTP 失败按 10 分钟、60 分钟退避，
第三次失败进入 `PERMANENT_FAILED`。正式邮件只渲染已保存报告，不重新调用 LLM。
